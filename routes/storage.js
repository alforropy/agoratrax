...
router.post(
  ...
  [
    ...
    check('viewmodal_harvest_logidSelect', 'Harvest ID value is not valid')
      ...
      .trim()
      .escape(),
    check('viewmodal_market_quantity', 'El valor de la cantidad de almacenamiento no es válido')
      .not()
      .isEmpty()
      .trim()
      .escape(),
    ...
    check('viewmodal_storage_Description', 'El valor de Descripción de almacenamiento no es válido')
      .not()
      .isEmpty()
      .trim()
      .escape(),
  ...
  function (req, res) {
    models.FoodprintStorage.findAll({
      ...
      .then(rows => {
        ...
        pdfService.buildPDF(
          `ENTRADAS DE ALMACENAMIENTO MAESTRO GENERADAS POR EL ADMINISTRADOR ${username} ${useremail}
  ...
  function (req, res) {
    models.FoodprintStorage.findAll({
      ...
      .then(rows => {
        ...
        pdfService.buildPDF(
          `ENTRADAS DE ALMACENAMIENTO PARA ${username} ${useremail}
                ${moment(new Date()).format('YYYY-MM-DD HH:mm:ss')}`,
          storagepdf(rows),
          chunk => stream.write(chunk),
          () => stream.end()
      ...
      .then(user => {
        ...
        models.FoodprintStorage.findAll({
          ...
          .then(rows => {
            ...
            let chunks = [];
            pdfService.buildPDF(
              `ENTRADAS DE ALMACENAMIENTO PARA ${user.firstName.toUpperCase()} ${user.lastName.toUpperCase()}
            ${moment(new Date()).format('YYYY-MM-DD HH:mm:ss')}`,
              storagepdf(rows),
